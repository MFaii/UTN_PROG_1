    ...
    if type(matriz_votos) == list and len(matriz_votos) > 0:
        ...
        for fil in range(len(matriz_votos)):
            for col in range(len(matriz_votos[fil])):
                ...
                voto = int(
                    input(
                        f"Ingrese la cantidad de votos del turno {col + 1} para el partido {fil + 1}: "
                    )
                )
...
def mostrar_partido(array_nombres: list, matriz_votos: list, indice: int) -> bool:
    if len(array_nombres) > indice and indice >= 0:
        porcentaje = calcular_porcentaje_votos(matriz_votos, indice)
...
# ESPECIFICA
def calcular_porcentaje_votos(matriz_votos: list, indice: int) -> float:
...
# Partidos con menos del 5% de votos: Mostrar los partidos que hayan recibido menos del 5% del total de votos. Si no hay ninguno, mostrar mensaje de error.


# ESPECIFICA/GENERAL - PARAMETRIZADA
def mostrar_partidos_que_no_superan_porciento(
    array_nombres: list, matriz_votos: list, porcentaje_menor: float
) -> bool:
    retorno = False

    for i in range(len(array_nombres)):
        # Calcular porcentaje
        porcentaje = calcular_porcentaje_votos(matriz_votos, i)
        # Verificar quien no supera ese porcentaje
